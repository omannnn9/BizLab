...
export function QuickLinksWidget({ widget, dashboardId }: { widget: DashboardWidget; dashboardId?: string }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      {links.length === 0 && !adding && (
        <p className="text-sm text-muted-foreground">Pin links to tools, docs, or sites your team uses often.</p>
      )}
      {links.length > 0 && (
          ...
          {links.map((link) => (
            <div key={link.id} className="group relative">
              <Button variant="outline" size="sm" asChild>
                <a href={link.url} target="_blank" rel="noopener noreferrer" className="pr-6">
              ...
              <button
                ...
                className="absolute right-1 top-1/2 -translate-y-1/2 rounded-full p-0.5 text-muted-foreground opacity-0 transition-opacity hover:bg-muted group-hover:opacity-100"
                title="Remove link"
              >
                <X className="size-3" />
              </button>
